...
이미 쓰고 있는 작은 서버에 사이드 프로젝트가 몇 개 올라가 있으면, 하나가 죽었을 때 알고 싶고 느려졌을 때는 *왜* 느린지 알고 싶습니다. 보통은 업타임 체커, 로그 뷰어, 호스팅 APM을 따로 쓰면서 화면을 오가며 같은 사건을 시간으로 맞춰 봐야 합니다. 무료 한도가 모자라서가 아니라(취미 규모에서는 대개 남습니다) 두 질문에 한 번에 답하고, 데이터를 내가 관리하는 서버에 두기 위해서입니다.
...
**http://localhost:3000** 을 열고 비밀번호를 정합니다. 모든 포트가 localhost에만 열리므로, 서버에 설치했다면 터널로 접속합니다. `ssh -N -L 3000:127.0.0.1:3000 you@your-server`를 쓰거나 [서버에서 운영하기](docs/getting-started.ko.md#서버에서-운영하기)의 다른 방법을 보세요. 호스트 지표는 15초 안에 들어오기 시작합니다. 트레이스·로그·에러를 바로 보고 싶다면 샘플 트래픽을 보내는 데모 가게(서비스 3개)를 켜세요.
...
합계는 약 440 MB이고, 데이터와 쿼리가 늘면 ClickHouse가 1 GiB 상한까지 커질 수 있습니다. 그래서 **메모리 2 GB**는 있는 장비를 권합니다. 1 GB VPS에서는 커널이 가장 먼저 ClickHouse를 죽입니다. 아주 작은 에이전트는 아닙니다.

## 나에게 맞을까?

minidog은 **한 사람이 계속 켜 두는 작은 서버에서 프로젝트 몇 개를 지켜보는 용도**입니다. VPS, NAS, 집에 둔 작은 컴퓨터 같은 것을 생각하면 됩니다. 사용자·권한·SSO가 없어서 팀용은 아니고, 기본적으로 localhost에서만 열리므로 인터넷에 공개하는 용도도 아닙니다.

노트북에서는 개발용으로 쓰기 좋습니다. 내 서비스의 트레이스와 로그를 몇 분 만에 볼 수 있습니다. 다만 **노트북을** 운영 감시용으로 쓰지는 마세요. 뚜껑을 닫으면 함께 잠드는 감시자는 새벽 3시에 사이트가 죽은 것을 알려 줄 수 없고, 나중에 "그 시간은 아무도 보고 있지 않았다"고 말해 줄 수 있을 뿐입니다(minidog 자신이 멈춘 것은 [외부 신호](docs/getting-started.ko.md#minidog-자신이-죽었을-때)로 알 수 있습니다). 다른 필요에는 이런 도구가 더 낫습니다.
