...
test('records an event only when the state changes', () => {
  ...
  const unchanged = monitors.recordEvaluation(
    monitors.get(monitor.id)!,
    { state: 'ok', value: 210, message: 'P95 210 ms' },
    at,
  );
...
test('pausing or resuming clears a pending transition', () => {
  ...
  assert.equal(
    monitors.recordEvaluation(monitor, critical, minutesLater(10)),
    null,
    'the delay starts again after resume',
  );
});
